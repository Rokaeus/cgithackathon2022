...
$AllComputers = Get-AzureADDevice -Filter * -Properties Name
...
Foreach ($ComputerName in $AllComputersNames) {
   ...
   $ComputerOS = Get-AzureADDevice $ComputerName -Properties OperatingSystem,OperatingSystemServicePack
   ...
   if ($Connection -eq "True"){
      $ComputerHW = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $ComputerName | Select-Object Manufacturer,Model,NumberOfProcessors,@{Expression={$_.TotalPhysicalMemory / 1GB};Label="TotalPhysicalMemoryGB"}
      $ComputerCPU = Get-WmiObject win32_processor -ComputerName $ComputerName | Select-Object DeviceID,Name,Manufacturer,NumberOfCores,NumberOfLogicalProcessors
      $ComputerDisks = Get-WmiObject -Class Win32_LogicalDisk -Filter "DriveType=3" -ComputerName $ComputerName | Select-Object DeviceID,VolumeName,@{Expression={$_.Size / 1GB};Label="SizeGB"}
...
$Inventory | Export-Csv C:\temp\Inventory.csv